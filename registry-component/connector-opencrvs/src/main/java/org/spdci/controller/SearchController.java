...
public class SearchController {
    ...
    @PostMapping(value = "/registry/search", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseWrapper> asysncSearch(@RequestHeader Map<String, String> headers, @RequestBody String requestBody)
            throws Exception {
        Payload payload = objectMapper.readValue(requestBody, Payload.class);
        template.convertAndSend(exchange, routingKey, payload);
        return Mono.just(new ResponseWrapper(AcknowledgementResponse.getAck()));
    ...
    @PostMapping(value = "/registry/sync/search", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseWrapper> syncSearch(@RequestHeader Map<String, String> headers, @RequestBody String requestBody)
            throws Exception {
        Payload payload = objectMapper.readValue(requestBody, Payload.class);
        log.info("search_request_received ", requestBody);
        ...
        return localCRVSService.search(payload)
                .flatMap(responsePayload -> {
                    ResponseWrapper responseWrapper = new ResponseWrapper(responsePayload);
                    return Mono.just(responseWrapper);
                });
