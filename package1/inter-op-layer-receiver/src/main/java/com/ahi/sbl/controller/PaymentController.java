...
public class PaymentController {
	...
	private final ResponseEntity<Mono<String>> error = ResponseEntity.of(Optional.ofNullable(Mono.just(
			new JsonFormat().printToString(SearchResponse.newBuilder().setErrorCode(HttpStatus.BAD_REQUEST.toString())
					.setReturnMessage("Error while searching").build()))));

	@PostMapping(value = "/v1/validateBankDetails", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Mono<String>> validateBankDetails(//
	...
	@PostMapping(value = "/v1/disbursement", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Mono<String>> disbursement(//
			@RequestHeader Map<String, String> headers, //
			@RequestBody String requestBody//
			, ServerHttpRequest request) {
